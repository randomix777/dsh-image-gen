/** X AI Grok subscription OAuth image adapter. */
...
const GROK_ASPECT_RATIOS: Record<string, string> = {
  '1024x1024': '1:1',
  '1024x1536': '2:3',
  '1536x1024': '3:2',
  auto: 'auto',
...
export async function generateGrokImage(input: {
  ...
  const response = await fetchWithRetry('https://api.x.ai/v1/images/generations', {
    ...
    body: JSON.stringify({
      ...
      ...(input.size.length > 0 ? { aspect_ratio: GROK_ASPECT_RATIOS[input.size] ?? input.size } : {}),
      ...(input.quality === 'low' ? { quality: 'low' } : input.quality === 'medium' ? { quality: 'medium' } : {}),
      n: input.count ?? 1,
    }),
  }, { signal: input.signal })
  const text = await readBoundedText(response, input.maxBytes * 2 + ERROR_LIMIT)
  if (!response.ok) throw new Error(`Grok image request failed (${response.status}): ${text.slice(0, ERROR_LIMIT)}`)
  let payload: unknown
  try { payload = JSON.parse(text) } catch { throw new Error('Grok image request returned invalid JSON') }
  const record = payload as { data?: Array<{ b64_json?: string; mime_type?: string }> }
  const entries = Array.isArray(record?.data) ? record.data : []
  if (entries.length === 0) throw new Error('Grok image request returned no image data')
  const entry = entries[0]
  if (!entry || typeof entry.b64_json !== 'string' || entry.b64_json.length === 0) throw new Error('Grok image request returned no base64 data')
  const decoded = Buffer.from(entry.b64_json.replace(/\s+/g, ''), 'base64')
  if (decoded.length === 0) throw new Error('Grok image request returned empty image data')
  if (decoded.byteLength > input.maxBytes) throw new Error(`Grok image exceeded ${input.maxBytes} bytes`)
