  ...
  const save = async (event: FormEvent): Promise<void> => {
    ...
    try {
      ...
      if (provider === 'comfyui') {
        ...
        await props.scope.set('agnesModel', model)
        await props.scope.set('agnesBaseURL', baseURL)
      } else if (provider === 'glm') {
        await props.scope.set('glmModel', model)
        await props.scope.set('glmBaseURL', baseURL)
      } else if (provider === 'stability') {
        await props.scope.set('stabilityModel', model)
        await props.scope.set('stabilityBaseURL', baseURL)
      } else {
      ...
      {open ? (
        <form className="dsh-ig-body" onSubmit={(event) => { void save(event) }}>
          ...
          <label className="dsh-ig-field">
            ...
            <span className="dsh-ig-hint">{provider === 'google' ? t('endpointHintGoogle') : provider === 'openai' ? t('endpointHintOpenAI') : provider === 'seedream' ? t('endpointHintSeedream') : provider === 'dashscope' ? t('endpointHintDashScope') : provider === 'comfyui' ? t('endpointHintComfyUI') : provider === 'agnes' ? t('endpointHintAgnes') : provider === 'glm' ? t('endpointHintGlm') : t('endpointHintStability')}</span>
          </label>
          {provider !== 'comfyui' ? <label className="dsh-ig-field">
...
function modelOf(provider: Provider, value: ImageSettings | undefined): string {
  const stored = provider === 'google' ? value?.googleModel : provider === 'openai' ? value?.openaiModel : provider === 'seedream' ? value?.seedreamModel : provider === 'dashscope' ? value?.dashscopeModel : provider === 'comfyui' ? value?.comfyuiWorkflowName : provider === 'agnes' ? value?.agnesModel : provider === 'glm' ? value?.glmModel : value?.stabilityModel
...
function baseURLOf(provider: Provider, value: ImageSettings | undefined): string {
  const stored = provider === 'google' ? value?.googleEndpoint : provider === 'openai' ? value?.openaiBaseURL : provider === 'seedream' ? value?.seedreamBaseURL : provider === 'dashscope' ? value?.dashscopeEndpoint : provider === 'comfyui' ? value?.comfyuiBaseURL : provider === 'agnes' ? value?.agnesBaseURL : provider === 'glm' ? value?.glmBaseURL : value?.stabilityBaseURL
